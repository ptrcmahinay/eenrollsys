...
$allAY = fetch_all('SELECT id, year_label, start_year FROM academic_years ORDER BY start_year DESC');
    ...
    <form method="post">
        ...
        <div style="margin-bottom:12px;">
            ...
            <div style="display:flex;gap:8px;">
                <input type="text" id="loa_student_no" name="student_no" style="flex:1;" placeholder="Enter student number..." required>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px;">
            <div><label style="font-size:13px;font-weight:600;">Name</label><input type="text" id="loa_student_name" readonly style="width:100%;background:#f1f5f9;" placeholder="Auto-filled after search"></div>
            <div><label style="font-size:13px;font-weight:600;">Program</label><input type="text" id="loa_student_program" readonly style="width:100%;background:#f1f5f9;" placeholder="Auto-filled after search"></div>
        </div>
        ...
        <div style="margin-bottom:12px;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                ...
                <div>
                    <select name="academic_year" style="width:100%;" required>
                        <option value="">-- Academic Year --</option>
                        <?php foreach ($allAY as $ay): ?>
                        <option value="<?= h($ay['year_label']) ?>"><?= h($ay['year_label']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
        ...
        <div style="margin-bottom:12px;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                ...
                <div>
                    <select name="expected_return_academic_year" style="width:100%;">
                        <option value="">-- Academic Year --</option>
                        <?php foreach ($allAY as $ay): ?>
                        <option value="<?= h($ay['year_label']) ?>"><?= h($ay['year_label']) ?></option>
                        <?php endforeach; ?>
...
function searchStudent() {
    ...
    if (!sno) { alert('Please enter a student number.'); return; }

    document.getElementById('loa_student_id').value = '0';
    document.getElementById('loa_student_name').value = '';
    document.getElementById('loa_student_program').value = '';
    document.getElementById('loa_program_id').value = '';
    document.getElementById('loa_department_id').value = '';

    fetch('../api/student_search.php?q=' + encodeURIComponent(sno))
        ...
        .then(function(data) {
            ...
            var s = data[0];
            document.getElementById('loa_student_id').value = s.id;
            document.getElementById('loa_student_no').value = s.student_number;
            document.getElementById('loa_student_name').value = s.full_name || '';
            document.getElementById('loa_student_program').value = s.program_name || '';
            document.getElementById('loa_program_id').value = s.program_id || '';
            document.getElementById('loa_department_id').value = s.department_id || '';
        })
